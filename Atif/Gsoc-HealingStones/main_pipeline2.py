...
class ReconstructionPipeline:
    ...
    def find_best_assembly_match(self, assembled, remaining):
        """Find best match between assembled and remaining fragments - IMPROVED"""
        best_match_info = None
        best_similarity = 0
        
        # Prioritize connections to most recently added fragments (chain building)
        assembled_list = list(assembled)
        
        # Sort assembled fragments by recency (assuming higher indices were added more recently)
        assembled_list.sort(reverse=True)
        
        for assembled_frag in assembled_list:
                ...
                if pair_key in self.all_matches:
                    ...
                    for color_matches in self.all_matches[pair_key].values():
                        for match in color_matches:
                            # Boost similarity for chain-like connections
                            connection_bonus = 0.0
                            if len(assembled) > 1:
                                # Prefer connecting to recently added fragments
                                if assembled_frag == max(assembled):
                                    connection_bonus = 0.1
                            
                            adjusted_similarity = match['similarity'] + connection_bonus
                            ...
                            if adjusted_similarity > best_similarity:
                                ...
                                best_match_info = {
                                    ...
                                    'match': match,
                                    'pair_key': pair_key,
                                    'base_similarity': match['similarity'],
    ...
    def find_non_overlapping_position(self, target_fragment, assembled_fragments, base_transform, max_attempts=20):
        """IMPROVED: Find a position for target fragment that doesn't overlap with assembled ones"""
        ...
        for assembled_frag in assembled_fragments:
            ...
            if len(assembled_points) > 0:
                ...
                overlapping, clearance = self.check_fragment_overlap(
                    transformed_points, assembled_transformed, min_clearance=0.01  # 1cm minimum
                )
        ...
        if not has_overlap:
            if self.config.get('debug_assembly', False):
                print(f"      Base position is clear (min clearance: {min_clearance*1000:.1f}mm)")
            return test_transform
        
        # If overlapping, try different positions
        if self.config.get('debug_assembly', False):
            print(f"      Base position overlaps (clearance: {min_clearance*1000:.1f}mm), trying alternatives...")
        
        # IMPROVED: Try more systematic positioning
        base_translation = base_transform[:3, 3]
        
        # Try positions along different axes and angles
        attempts = [
            # Linear offsets along major axes
            np.array([0.05, 0, 0]),    # +X
            np.array([-0.05, 0, 0]),   # -X  
            np.array([0, 0.05, 0]),    # +Y
            np.array([0, -0.05, 0]),   # -Y
            np.array([0, 0, 0.02]),    # +Z (small vertical)
            np.array([0, 0, -0.02]),   # -Z
            # Diagonal offsets
        ...
        # Add radial pattern for remaining attempts
        for i in range(len(attempts), max_attempts):
            angle = (i * 2 * np.pi) / 12  # 12 positions around circle
            radius = 0.03 + (i * 0.02)   # Increasing radius
            
            offset = np.array([
                radius * np.cos(angle),
                radius * np.sin(angle),
                0.01 * (i // 12)  # Slight vertical stacking only as last resort
            ])
            attempts.append(offset)
        
        for attempt_idx, offset in enumerate(attempts[:max_attempts]):
            test_transform = base_transform.copy()
            ...
            # Check this position
            position_clear = True
            min_clearance_this_pos = float('inf')
            ...
            for assembled_frag in assembled_fragments:
                ...
                if len(assembled_points) > 0:
                    ...
                    overlapping, clearance = self.check_fragment_overlap(
                        transformed_points, assembled_transformed, min_clearance=0.01
                    )
                    min_clearance_this_pos = min(min_clearance_this_pos, clearance)
                    if overlapping:
            ...
            if position_clear:
                if self.config.get('debug_assembly', False):
                    print(f"      Found clear position at attempt {attempt_idx+1}")
                    print(f"      Offset: {np.linalg.norm(offset)*1000:.1f}mm, clearance: {min_clearance_this_pos*1000:.1f}mm")
                return test_transform
        
        # If no clear position found, use best attempt with warning
        if self.config.get('debug_assembly', False):
            print(f"      Warning: No completely clear position found")
            print(f"      Using last attempt with best available clearance")
    ...
    def improved_align_component(self, fragment_indices):
        ...
        if self.config.get('debug_assembly', False):
            ...
            print(f"   Strategy: Chain-based progressive assembly with overlap prevention")
        ...
        contact_distances = []
        connection_count = {}  # Track how many fragments are connected to each fragment
        
        # Initialize connection count
        for frag_idx in fragment_indices:
            connection_count[frag_idx] = 0
        
        if self.config.get('debug_assembly', False):
            print(f"   Origin fragment: {origin_fragment}")
            print(f"   Remaining fragments: {remaining}")
        
        # Step 2: Chain-based progressive assembly with overlap prevention
        while remaining and (time.time() - start_time) < max_time:
            ...
            base_similarity = best_match_info['base_similarity']
            connection_bonus = best_match_info.get('connection_bonus', 0.0)
            
            # CHAIN BUILDING LOGIC: Prefer fragments with fewer connections
            if connection_count[assembled_frag_idx] >= 2:
                if self.config.get('debug_assembly', False):
                    print(f"   Fragment {assembled_frag_idx} already has {connection_count[assembled_frag_idx]} connections")
                    print(f"   Looking for alternative connection point...")
                
                # Find alternative assembly point with fewer connections
                alternative_found = False
                for af in assembled_fragments:
                    alt_frag_idx = af['index']
                    if connection_count[alt_frag_idx] < 2:  # Prefer fragments with < 2 connections
                        alt_pair_key = f"fragment_{min(alt_frag_idx, target_frag_idx)}_to_{max(alt_frag_idx, target_frag_idx)}"
                        
                        if alt_pair_key in self.all_matches:
                            for color_matches in self.all_matches[alt_pair_key].values():
                                for alt_match in color_matches:
                                    if alt_match['similarity'] >= 0.5:  # Minimum acceptable similarity
                                        if self.config.get('debug_assembly', False):
                                            print(f"   Found alternative: Fragment {target_frag_idx} to {alt_frag_idx} (sim: {alt_match['similarity']:.3f})")
                                        
                                        # Use alternative connection
                                        assembled_frag_idx = alt_frag_idx
                                        match = alt_match
                                        best_match_info['assembled_fragment'] = alt_frag_idx
                                        best_match_info['match'] = alt_match
                ...
                if not alternative_found and self.config.get('debug_assembly', False):
                    print(f"   No suitable alternative found, proceeding with original connection")
            
            if self.config.get('debug_assembly', False):
                print(f"\n   Assembling fragment {target_frag_idx} to {assembled_frag_idx}")
                print(f"   Match similarity: {base_similarity:.3f} (bonus: +{connection_bonus:.3f})")
                print(f"   Connection count for {assembled_frag_idx}: {connection_count[assembled_frag_idx]}")
            ...
            connection_count[assembled_frag_idx] += 1
            ...
            if self.config.get('debug_assembly', False):
                ...
                print(f"   Updated connection counts: {connection_count}")
        ...
        assembly_quality = {
            ...
            'connection_distribution': connection_count
        }
        ...
        if self.config.get('debug_assembly', False):
            print(f"\n   🔧 FIXED Chain Assembly completed in {total_time:.1f}s")
            print(f"   Assembly order: {assembly_order}")
            print(f"   Success rate: {assembly_quality['assembly_success_rate']*100:.1f}%")
            print(f"   Final connection distribution: {connection_count}")
